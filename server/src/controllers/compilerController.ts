...
import { IAuthRequest } from "../middlewares/verifyToken";
import { User } from "../models/User";

export const saveCode = async (req: IAuthRequest, res: Response) => {
  const { fullCode, title }: { fullCode: IFullCode; title: string } = req.body;
...
export const loadCode = async (req: IAuthRequest, res: Response) => {
  const { id } = req.query;
...
export const getMyCodes = async (req: IAuthRequest, res: Response) => {
  const userId = req._id;
  try {
...
export const deleteCode = async (req: IAuthRequest, res: Response) => {
  const userId = req._id;
  const { id } = req.params;
  try {
...
export const editCode = async (req: IAuthRequest, res: Response) => {
  const userId = req._id;
